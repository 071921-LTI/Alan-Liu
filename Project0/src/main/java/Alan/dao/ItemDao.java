...
public class ItemDao {
	...
	public Items getItemById(int id) throws SQLException {
		...
		try{
			...
			log.error("Failed to find item with Id " + id + e.fillInStackTrace());
			e.printStackTrace();
		}
		finally {
			conn.close();
	...
	public boolean addItems(Items items) throws SQLException
	{
		...
		String sql = "INSERT INTO Items(Iname,price,quantity) VALUES(?,?,?)";
			...
			{
				log.error("Failed to add item"+ e.fillInStackTrace());
				e.printStackTrace();
			}
			finally {
				conn.close();
	...
	public ArrayList<Items> DisplayItem() throws SQLException {
		...
		try
		{
			...
			log.error("Failed to Display item"+ e.fillInStackTrace());
			e.printStackTrace();
		}
		finally {
			conn.close();
	...
	public boolean UpdatePrice(int id,double price) throws SQLException {
		...
		try
		{
			...
			log.error("Failed to Update item Price"+ e.fillInStackTrace());
			e.printStackTrace();
		}
		finally {
			conn.close();
...
public boolean UpdateQuantity(int id,int n) throws SQLException {
		...
		try
		{
			...
			log.error("Failed to Update item Quantity"+ e.fillInStackTrace());
			e.printStackTrace();
		}
		finally {
			conn.close();
